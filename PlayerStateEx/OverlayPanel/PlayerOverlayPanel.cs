...
public partial class PlayerOverlayPanel : Control
{
    ...
    private int _layoutDelay; // 延迟计数器，等待多帧后再更新高度
    ...
    public override void _Process(double delta)
    {
        if (_needsRefresh)
        {
            Log.Debug("_Process: _needsRefresh=true, calling RefreshAllProviders");
            _needsRefresh = false;
            if (_player != null) RefreshAllProviders();
        }

        // 延迟更新高度：等待 Godot 布局系统完成计算
        if (_layoutDelay > 0)
        {
            _layoutDelay--;
            if (_layoutDelay == 0)
            {
                Log.Debug("_Process: layout delay done, calling UpdatePanelHeight");
                UpdatePanelHeight();
            }
        }
    }
    ...
    private void UpdatePanelHeight()
    {
        ...
        // 调试：统计子节点数
        var childCount = _contentContainer.GetChildCount();
        var contentSize = _contentContainer.Size;
        var minSize = _contentContainer.GetMinimumSize();

        Log.Debug($"UpdatePanelHeight: children={childCount}, Size.Y={contentSize.Y}, MinSize.Y={minSize.Y}");

        // 使用 GetMinimumSize() 获取实时所需高度，而非 Size.Y（可能是旧的）
        var contentHeight = minSize.Y;
        ...
        Log.Debug($"targetHeight={targetHeight}, maxHeight={maxHeight}");
        ...
        _mainContainer.Size = new Vector2(PanelWidth, 0);
    }
    ...
    private void RefreshAllProviders()
    {
        ...
        Log.Debug("RefreshAllProviders: start");

        ClearProviderContents();
        CreateProviderContents();
        // 延迟更新高度到下一帧（等待布局系统计算完成）
        _layoutDelay = 2;
        PanelPositionHelper.ClampToViewport(_panel);
    }
    ...
    private void OnProviderContentChanged(IPlayerPanelProvider provider)
    {
        ...
        Log.Debug($"OnProviderContentChanged: provider={provider.ProviderId}");
